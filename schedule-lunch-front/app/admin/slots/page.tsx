...
export default function AdminSlotsPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-bold text-white mb-6">{t.slotManagement}</h1>
      <form onSubmit={create} className="bg-gray-900 p-4 rounded-xl border border-gray-800 mb-8 grid grid-cols-2 gap-3">
        <h2 className="col-span-2 text-sm text-gray-400 font-medium">{t.newSlot}</h2>
        ...
        {slots.map(s => (
          <li key={s.id} className="flex items-center justify-between bg-gray-900 p-3 rounded-lg border border-gray-800">
            <div className="text-sm">
              ...
              <span className="text-gray-500 ml-2">{s.attendeeCount} {t.reservationsLabel}</span>
            </div>
            <button onClick={() => remove(s.id)} className="text-red-500 hover:text-red-400 text-sm">{t.deleteAction}</button>
          </li>
        ))}
      </ul>
    </div>
