...
export default function AdminGroupsPage() {
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-xl font-bold text-white mb-6">{t.groupsTitle}</h1>
      <form onSubmit={create} className="flex flex-col gap-2 mb-8 bg-gray-900 p-4 rounded-xl border border-gray-800">
        <h2 className="text-sm text-gray-400 font-medium mb-1">{t.newGroup}</h2>
      ...
      <ul className="flex flex-col gap-2">
        {groups.map(g => (
          <li key={g.id} className="flex items-center justify-between bg-gray-900 p-3 rounded-lg border border-gray-800">
            ...
            <button onClick={() => remove(g.id)} className="text-red-500 hover:text-red-400 text-sm">{t.deleteAction}</button>
          </li>
        ))}
      </ul>
    </div>
